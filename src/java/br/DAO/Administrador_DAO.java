...
public class Administrador_DAO {
    ...
    public void Inserir(Administrador adm) throws Exception {
        Conexao conexao = new Conexao();
         try {
            PreparedStatement sql = conexao.getConexao().prepareStatement("INSERT INTO ADMINISTRADOR (loggin,senha) VALUES (?,?)");
            sql.setString(1, adm.getLoggin());
    ...
    public void Alterar(Administrador adm) throws Exception {
        Conexao conexao = new Conexao();
        try {
            PreparedStatement sql = conexao.getConexao().prepareStatement("UPDATE ADMINISTRADOR SET loggin= ?, senha =? WHERE ID = ? ");
            sql.setString(1, adm.getLoggin());
            sql.setString(2, adm.getSenha());
